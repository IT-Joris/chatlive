<!-- Component Start -->
<div class="flex flex-col flex-grow w-full max-w-xl bg-white dark:bg-gray-700 shadow-xl rounded-lg overflow-hidden" style="height: 70vh">
    <div class="flex flex-col flex-grow h-0 p-4 overflow-auto" id="chatAppScroll">
        @forelse($messages as $message)
            @if($message->user_id == auth()->user()->id)
                <div class="flex w-full mt-2 space-x-3 max-w-xs ml-auto justify-end">
                    <div>
                        <div class="bg-blue-500 text-white p-3 rounded-l-lg rounded-br-lg @if(str_starts_with($message->content, 'file::')) dark:bg-gray-800 @endif">
                            @if(str_starts_with($message->content, 'file::'))
                                <a href="{{ url(str_replace('file::', '', $message->content)) }}" target="_blank">
                                    <div id="toast-message-cta" class="w-full max-w-fit p-4 text-gray-500 bg-white rounded-lg shadow dark:bg-gray-800 dark:text-gray-400" role="alert">
                                        <div class="flex">
                                            <div class="ml-3 text-sm font-normal">
                                                <span class="mb-1 text-sm font-semibold text-gray-900 dark:text-white">Téléchargement</span>
                                                <div class="mb-2 text-sm font-normal">{{ $message->user->name }} a envoyé un fichier</div>
                                                <a href="#" class="inline-flex px-2.5 py-1.5 text-xs font-medium text-center text-white bg-blue-600 rounded-lg hover:bg-blue-700 focus:ring-4 focus:outline-none focus:ring-blue-300 dark:bg-blue-500 dark:hover:bg-blue-600 dark:focus:ring-blue-800">Appuiyer pour télécharger</a>
                                            </div>
                                        </div>
                                    </div>
                                </a>
                            @else
                                <p class="text-sm">{!! nl2br($message->content) !!}</p>
                            @endif
                        </div>
            ...
            @if($message->user_id != auth()->user()->id)
                <div class="flex w-full mt-2 space-x-3 max-w-xs">
                    <div class="flex-shrink-0 h-10 w-10 rounded-full bg-gray-300"></div>
                    <div>
                        <div class="bg-gray-500 text-white p-3 rounded-l-lg rounded-br-lg @if(str_starts_with($message->content, 'file::')) dark:bg-gray-800 @endif">
                            @if(str_starts_with($message->content, 'file::'))
                                <a href="{{ url(str_replace('file::', '', $message->content)) }}" target="_blank">
                                    <div id="toast-message-cta" class="w-full max-w-fit p-4 text-gray-500 bg-white rounded-lg shadow dark:bg-gray-800 dark:text-gray-400" role="alert">
                                        <div class="flex">
